import { presentationID, setPresentationID, auth } from "./globals.js";
/**
 ...
 */
async function addNewParagraphSlide(title, content, auth) {
    if (!presentationID) {
        throw new Error("addnewParagraphSlide(): 'presentationID' {string} does not exist! Have you created a presentation yet?");
    }
    if (!auth) {
        throw new Error("addNewParagraphSlide(): 'auth' {slides_v1.Slide} is not set!");
    }
    ...
    if (!slidesArray) {
        throw new Error(`addNewParagraphSlide(): Could not retrieve a reference to the slide array`);
    }
    const currentSlide = slidesArray[slidesArray.length - 1];
    if (!currentSlide ||
        !currentSlide.pageElements ||
        currentSlide.pageElements.length === 0) {
        throw new Error("addNewParagraphSlide(): Recently created slide not found or has no elements");
    }
 ...
 */
async function addNewBulletSlide(title, content, auth) {
    if (!presentationID) {
        throw new Error("addNewBulletSlide(): 'presentationID' {string} does not exist! Have you created a presentation yet?");
    }
    if (!auth) {
        throw new Error("addNewBulletSlide(): 'auth' {slides_v1.Slide} is not set!");
    }
    //Adds a new slide in our presentation with a unique ID
    const uniqueID = `id_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
    const newSlide = await auth.presentations.batchUpdate({
    ...
    const slidesArray = presentation.data.slides;
    if (!slidesArray) {
        throw new Error(`addNewBulletSlide(): Could not retrieve a reference to the slide array`);
    }
    const currentSlide = slidesArray[slidesArray.length - 1];
    if (!currentSlide ||
        !currentSlide.pageElements ||
        currentSlide.pageElements.length === 0) {
        throw new Error("addNewBulletSlide(): Recently created slide not found or has no elements");
    }
    //There are automatically two textbox elements in this layout.
    //Get the ID's of those SHAPES.
    const titleId = currentSlide.pageElements[0].objectId;
 ...
 */
export async function addCustomSlide(title, content, presentationId, type) {
    try {
        //Checks that a 'slides' instance exists
        if (!auth) {
            throw new Error("addCustomSlide(): A slides_v1.Slides object does not exist. Did you create a presentation?");
        }
        //Sets the presentationID
        if (!presentationId) {
            throw new Error("addCustomSlide(): The inputted 'presentationId' is not properly set.");
        }
        setPresentationID(presentationId);
        //Creates the appropriate type of slide depending on 'type'
        switch (type) {
            case "Paragraph":
                await addNewParagraphSlide(title, content, auth);
                break;
            case "Bullet":
                await addNewBulletSlide(title, content, auth);
                break;
            default:
                throw new Error(`addCustomSlide(): You must input a proper 'SlideType' string. ${type} is not valid`);
        }
    }
    catch (error) {
        throw new Error(`addCustomSlide(): Could not create a new slide: \n${error}`);
    }
}
