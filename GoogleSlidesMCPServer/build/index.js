...
server.tool("extract-data", extractDataToolDescription, {
    name: z
        .string()
        .describe("The name (or the first property) of the row of data we want to extract."),
    ...
    try {
        ...
        const selectedRow = rows.find((row) => row[0].trim().toLowerCase() === name.trim().toLowerCase());
        if (!selectedRow) {
            return {
                content: [
                    {
...
server.tool("create-presentation", createPresentationToolDescription, {
    companyName: z
        .string()
    ...
    try {
        ...
        console.error("create-presentation: Error creating presentation:\n", error);
        return {
            content: [
                {
                    type: "text",
...
server.tool("add-custom-slide", addCustomSlideToolDescription, {
    ...
    slideType: z
        .string()
        .describe("One of the literal strings 'Paragraph' or 'Bullet' that specifies what kind of slide this tool will create"),
