...
async function addNewBulletSlide(
  ...
  if (!presentationID) {
    throw new Error(
      "addNewBulletSlide(): 'presentationID' {string} does not exist! Have you created a presentation yet?"
    );
  }
  if (!auth) {
    throw new Error(
      "addNewBulletSlide(): 'auth' {slides_v1.Slide} is not set!"
    );
  }
  ...
  if (!slidesArray) {
    throw new Error(
      `addNewBulletSlide(): Could not retrieve a reference to the slide array`
    );
  }
  ...
  if (
    ...
    throw new Error(
      "addNewBulletSlide(): Recently created slide not found or has no elements"
    );
  }
...
export async function addCustomSlide(
  ...
  try {
    ...
    if (!auth) {
      throw new Error(
        "addCustomSlide(): A slides_v1.Slides object does not exist. Did you create a presentation?"
      );
    }
